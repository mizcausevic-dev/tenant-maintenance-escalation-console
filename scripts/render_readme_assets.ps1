...
$port = 5442
$process = $null
$stdout = Join-Path $env:TEMP ("tenant-maintenance-escalation-console-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("tenant-maintenance-escalation-console-" + [guid]::NewGuid().ToString() + "-stderr.log")
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/maintenance-lane"; File = "02-maintenance-lane-proof.png"; Size = "1600,1400" },
        @{ Url = "http://127.0.0.1:$port/escalation-queue"; File = "03-escalation-queue-proof.png"; Size = "1600,1320" },
